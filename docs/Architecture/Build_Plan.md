...
- [~] Roles: Owner, Co-founder, Admin, Manager, Captain, Scorer, Media Manager, Member/Viewer — _WP-C1: `ClubRole` enum defined (full §7 set); only Owner assigned so far (on club creation). Role management UI pending._
...
- [~] Frontend: club creation, members management, club profile — _WP-C1: "clubs near you" discovery finder shipped; create-club UI intentionally hidden in V1 (endpoint exists), members mgmt + club profile pending._
...
- [~] Frontend: player profile (basic card), ghost claim screen — _creation flow shipped (WP-20260702, reworked WP-20260819); a first player card ships as the setup reveal; standalone profile page + ghost claim still pending._
