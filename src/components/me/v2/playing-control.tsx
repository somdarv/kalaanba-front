...
/**
 * Availability — the one control on this surface that writes on a single tap.
 *
 * It earns that because of Player & Affiliation §12: a player's availability
 * feeds the club readiness summary, so this is the field with a consequence
 * past this page. Everything else here is read-only or edited behind a sheet.
 *
...
export type PlayingControlProps = {
  player: MyPlayer;
  meta: PlayerMeta;
};

export function PlayingControl({ player, meta }: PlayingControlProps) {
  const { push } = useToast();
  ...
  return (
    <div className="mt-1">
      <p className="text-fg-muted text-sm">Clubs near you see this.</p>
      ...
      {/* The config author's own line for the selected option. `aria-live` so
          a screen reader hears the description change after a tap, which is
          the only feedback the toast does not already carry. */}
      <p className="text-fg-subtle mt-3 text-sm" aria-live="polite">
        {currentDescription ??
